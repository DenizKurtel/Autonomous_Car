        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence > 0.5:
                ...
                if self.classes[idx] in self.relevant:
                    ...
                    if self.classes[idx] in self.relevant and confidence > 0.8:
                        self.stop_flag = True
                    else:
                        self.stop_flag = False
...
while True:
    ...
    traffic_frame_ = frame
    lane.drive(frame)
    obj.detect(frame)

    # i = i + 1
    # if i % 11 == 0:
    #     traffic_frame_ = traffic.signs(traffic_frame=traffic_frame_, detection_graph=traffic_detection_graph,
    #                                    category_index=traffic_category_index,
    #                                    categories=traffic_categories, car_speed=car.speed)
        # print(type(traffic_frame_))
        # c
        # v2.imshow('Traffic Window', traffic_frame_)
        # i = 0
    cv2.imshow('Object Window', frame)
    car.drive(speed=car.speed, steering_angle=lane.stabilized_steering_angle, obj_flag=obj.stop_flag, lane_flag=lane.stop_flag, traffic_flag=traffic.traffic_flag, max_speed=traffic.max_speed)
