    ...
    def detect(self, frame):
        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence > 0.2:
                ...
                if (abs(startX - endX) > 80) and self.classes[idx] == "person":
                    self.stop_flag = True
                else:
                    self.stop_flag = False
...
class AutonomousCar:
    ...
    def drive(self, speed, steering_angle, flag):
        if flag:
            self.speed = 0
        else:
...
while True:
    ...
    receive_elapsed = time.time() - receive_time

    lane_time = time.time()
    lane.drive(frame)
    lane_elapsed = time.time() - lane_time
    obj_time = time.time()
    obj.detect(frame)
    obj_elapsed = time.time() - obj_time

    send_time = time.time()
    car.drive(speed=car.speed, steering_angle=lane.stabilized_steering_angle, flag=obj.stop_flag)
    stabilized = str(car.steering_angle)
    speed = str(car.speed)
    package = stabilized.encode() + str('/').encode() + speed.encode()
    conn.send(package)
    send_elapsed = time.time() - send_time
